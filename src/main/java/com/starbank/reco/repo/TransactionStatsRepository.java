...
""";
        var params = new MapSqlParameterSource().addValue("userId", userId.toString());
        ...
        return rows.stream().collect(Collectors.toMap(r -> r.productType, r -> new TxAgg(r.depositSum, r.spendSum, r.txCount)));
    }
...
WHERE t.USER_ID = CAST(:userId AS UUID) AND UPPER(p."TYPE") = :ptype
""";
        var p = new MapSqlParameterSource().addValue("userId", userId.toString()).addValue("ptype", productType.toUpperCase());
...
WHERE t.USER_ID = CAST(:userId AS UUID) AND UPPER(p."TYPE") = :ptype
""";
        var p = new MapSqlParameterSource().addValue("userId", userId.toString())
                .addValue("ptype", productType.toUpperCase()).addValue("tx", txType.toUpperCase());
...
""";
        var p = new MapSqlParameterSource().addValue("userId", userId.toString()).addValue("ptype", productType.toUpperCase());
        return jdbc.query(sql, p, rs -> rs.next() ? new Dw(nz(rs.getBigDecimal("dep")), nz(rs.getBigDecimal("wdr"))) : new Dw(BigDecimal.ZERO, BigDecimal.ZERO));
    }
